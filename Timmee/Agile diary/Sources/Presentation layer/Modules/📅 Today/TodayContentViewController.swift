...
final class TodayContentViewController: UIViewController, AlertInput {
    ...
    var sprint: Sprint! {
        didSet {
            guard sprint.id != oldValue?.id else { return }
...
private extension TodayContentViewController {
    ...
    func setupCurrentCacheObserver() {
        guard isViewLoaded, view.window != nil, !view.isHidden, sprint != nil else { return }

        switch section {
        case .habits: setupHabitsCacheObserver(forSection: section, sprintID: sprint.id)
        case .goals: setupGoalsCacheObserver(forSection: section, sprintID: sprint.id)
        }
    }
...
private extension TodayContentViewController {
    ...
    func hidePlaceholder() {
        ...
        placeholderView.setVisible(false, animated: false)
        allHabitsButton.isHidden = false
    }
...
private extension TodayContentViewController {
    ...
    func setupTargetCellActionsProvider() {
        ...
        targetCellActionsProvider.onDone = { [unowned self] indexPath in
            ...
            self.goalsService.updateGoal(goal, sprintID: self.sprint.id, completion: { _ in })
...
private extension TodayContentViewController {
    ...
    func setupAllHabitsButton() {
        ...
        allHabitsButton.setTitle("edit".localized, for: .normal)
        ...
        allHabitsButton.backgroundColor = AppTheme.current.colors.foregroundColor
        allHabitsButton.layer.cornerRadius = 8
        allHabitsButton.configureShadow(radius: 8, opacity: 0.2)
    ...
    @objc func didTapAllHabitsButton() {
        transitionHandler?.present(AllHabitsViewController(sprint: sprint), animated: true, completion: nil)
    }
