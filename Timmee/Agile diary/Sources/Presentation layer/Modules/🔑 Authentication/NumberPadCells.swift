...
class BasePadCell: UICollectionViewCell {
    ...
    var highlightedBackgroundColor: UIColor = AppTheme.current.colors.decorationElementColor.withAlphaComponent(0.5)
...
final class NumberPadCell: BasePadCell {
    ...
    override func configure(with item: NumberPadItem) {
        ...
        if case .clear = item.kind {
            titleLabel.textColor = AppTheme.current.colors.wrongElementColor
        } else {
...
final class IconPadCell: BasePadCell {
    ...
    override func configure(with item: NumberPadItem) {
        ...
        if case .clear = item.kind {
            iconView.tintColor = AppTheme.current.colors.wrongElementColor
        } else {
            iconView.tintColor = AppTheme.current.colors.activeElementColor
        }
    }
