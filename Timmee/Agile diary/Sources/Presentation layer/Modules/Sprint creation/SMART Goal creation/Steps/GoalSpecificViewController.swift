//
//  GoalSpecificViewController.swift
//  Agile diary
//
...
protocol GoalSpecificOutput: AnyObject {
    func didAskToContinue(specific: String)
}

final class GoalSpecificViewController: BaseViewController {
    
    weak var output: GoalSpecificOutput?
    ...
    override func refresh() {
        ...
        titleLabel.text = "goal_specific_screen_title".localized
        inputField.placeholderAttributedText
            = NSAttributedString(string: "goal_specific_input_placeholder".localized,
                                 attributes: [.font: AppTheme.current.fonts.medium(20),
                                              .foregroundColor: AppTheme.current.colors.inactiveElementColor])
        hintLabel.text = "goal_specific_hint".localized
...
private extension GoalSpecificViewController {
...
private extension GoalSpecificViewController {
    ...
    @objc func onTapContinueButton() {
        guard let specific = inputField.textView.text.nilIfEmpty else {
            return
        }
        
        output?.didAskToContinue(specific: specific)
    }
...
extension GoalSpecificViewController: UITextViewDelegate {
