...
@objc public protocol ReorderableTableViewDelegate: NSObjectProtocol {
    ...
    @objc optional func tableView(_ tableView: UITableView, reorderRowsFrom fromIndexPath: IndexPath, to toIndexPath: IndexPath)
	...
	*/
	...
	fileprivate func initialize() {
		longPressGestureRecognizer = UILongPressGestureRecognizer(target: self,
                                                                  action: #selector(ReorderableTableView._longPress(_:)))
...
fileprivate extension ReorderableTableView {
	
    func canMoveRowAt(indexPath: IndexPath) -> Bool {
	...
	@objc func _longPress(_ gesture: UILongPressGestureRecognizer) {
		...
		// Dropped.
			...
			UIView.animate(withDuration: 0.3,
				...
				completion: { [unowned self] (Bool) -> Void in
                    ...
                    if let initialIndexPath = self.initialIndexPath, let currentLocationIndexPath = self.currentLocationIndexPath {
                        self.longPressReorderDelegate?.tableView?(self,
                                                                  reorderRowsFrom: initialIndexPath,
                                                                  to: currentLocationIndexPath)
                    }
	...
	@objc func _scrollTableWithCell(_ sender: CADisplayLink) {
    ...
    func newYCenter(for draggingView: UIView, with location: CGPoint) -> CGFloat {
